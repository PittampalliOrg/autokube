<script lang="ts">
	...
	import { TableName, rolesColumns } from '$lib/table-columns';
	...
	let selectedRole = $state<RoleWithAge | null>(null);
	...
	// Roles with age
	const rolesWithAge = $derived.by((): RoleWithAge[] => {
		const currentTime = timeTicker.now;
		return allRoles.map((role) => ({
			...role,
			age: calculateAgeWithTicker(role.createdAt, currentTime)
		}));
	});

	// Filtered
	const filteredRoles = $derived.by(() => {
		let result = rolesWithAge;

		if (selectedNamespace !== 'all') {
			result = result.filter((role) => role.namespace === selectedNamespace);
		}
		...
		if (sortState) {
			result = arraySort(result, sortState.field as keyof Role, sortState.direction, {
	...
	async function fetchNamespaces() {
		...
		try {
			...
			console.error('[Roles] Failed to fetch namespaces:', err);
		}
	}
	...
	async function handleDelete(name: string, namespace: string) {
		...
		try {
			...
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/roles/${encodeURIComponent(name)}?namespace=${encodeURIComponent(namespace)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
	...
	function openDetail(role: RoleWithAge) {
		selectedRole = role;
		showDetailDialog = true;
	}

	function openYamlEditor(role: RoleWithAge) {
		drawerResource = { resourceType: 'role', name: role.name, namespace: role.namespace };
		showYamlDialog = true;
	}
	...
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			...
			<span class="text-sm text-muted-foreground">
				{filteredRoles.length} of {rolesWithAge.length}
			</span>
			<Button
				variant="outline"
				size="sm"
	...
	{#if !loading && !error && !activeCluster}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			...
			<div>
				...
				<p class="text-sm text-muted-foreground">Select a cluster to view roles</p>
			</div>
		</div>
	{:else}
			...
			>
				{#snippet cell(column, role: RoleWithAge, rowState)}
					...
					{:else if column.id === 'rules'}
						<span class="text-xs font-medium">{role.rules.length}</span>
					{:else if column.id === 'age'}
						<span class="text-xs text-muted-foreground">{role.age}</span>
					{:else if column.id === 'actions'}
						<div class="flex items-center justify-end gap-0.5">
							...
							<ConfirmDelete
								title={role.name}
								loading={deleting}
								onConfirm={() => handleDelete(role.name, role.namespace)}
							>
								<Button
									variant="ghost"
									size="icon"
				...
				{#snippet emptyState()}
					<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
						...
						<h3 class="mb-1 font-semibold text-foreground">No roles found</h3>
						<p class="text-sm">
							{selectedNamespace === 'all'
								? 'This cluster has no roles'
								: `No roles in namespace "${selectedNamespace}"`}
						</p>
					</div>
				{/snippet}
...
<Dialog.Root bind:open={showDetailDialog}>
	<Dialog.Content class="max-h-[90vh] max-w-4xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title>Role Details</Dialog.Title>
		</Dialog.Header>
		{#if selectedRole}
			<div class="space-y-6">
				<!-- Overview -->
				<div>
					...
					<div class="grid grid-cols-2 gap-4">
						<div>
							...
							<p class="mt-1 font-mono text-sm">{selectedRole.name}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Namespace</p>
							<Badge variant="outline" class="mt-1 text-xs">{selectedRole.namespace}</Badge>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Rules</p>
							<p class="mt-1 text-sm font-medium">{selectedRole.rules.length}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Age</p>
							<p class="mt-1 text-sm">{selectedRole.age}</p>
						</div>
						<div class="col-span-2">
							<p class="text-sm font-medium text-muted-foreground">Created</p>
							<p class="mt-1 font-mono text-sm">
								{formatCreatedAt(selectedRole.createdAt)}
							</p>
						</div>
					</div>
				</div>
				...
				<!-- Rules -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Rules</h3>
					{#if selectedRole.rules.length > 0}
						<div class="max-h-72 space-y-2 overflow-y-auto">
							{#each selectedRole.rules as rule, i}
								<div class="rounded-md border bg-muted/40 px-3 py-2">
									<div class="grid grid-cols-3 gap-2 text-xs">
										<div>
											<span class="font-medium text-muted-foreground">API Groups</span>
											<p class="mt-0.5 font-mono">{formatApiGroups(rule.apiGroups)}</p>
										</div>
										<div>
											<span class="font-medium text-muted-foreground">Resources</span>
											<p class="mt-0.5 font-mono">{formatResources(rule.resources)}</p>
										</div>
										<div>
											<span class="font-medium text-muted-foreground">Verbs</span>
											<p class="mt-0.5 font-mono">{formatVerbs(rule.verbs)}</p>
										</div>
									</div>
								</div>
							{/each}
				...
				<div>
					...
					{#if Object.keys(selectedRole.labels ?? {}).length > 0}
						<div class="max-h-48 space-y-1 overflow-y-auto">
							{#each Object.entries(selectedRole.labels) as [k, v]}
				...
				<div>
					...
					{#if Object.keys(selectedRole.annotations ?? {}).length > 0}
						<div class="max-h-72 space-y-2 overflow-y-auto pr-1">
							{#each Object.entries(selectedRole.annotations) as [k, v]}
